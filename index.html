...
<body>
  ...
  <script>
    const levels = [
      ['HHHH1', 'Power Grid', 'Learn the maze and activate the switches.'],
      ['HHHH2', 'Pure Chase', 'Outrun the enemy in a tighter chamber.'],
      ['HHHH3', 'Frozen Orb', 'Use the frozen orb to escape pursuit.'],
      ['HHHH4', 'Power Grid Plus', 'A tougher power-grid route.'],
      ['HHHH5', 'Shield & Barrier', 'Place barriers and break through.'],
      ['HHHH6', 'Code Gate', 'Find the key and open the final gate.'],
      ['HHHH7', 'Final Gate', 'Survive the first Hell Mode chamber.'],
      ['HHHH8', 'Final Gate Plus', 'More hazards, less room to recover.'],
      ['HHHH9', 'Hell Mode', 'A fast, hostile final maze.'],
      ['HHHH10', 'Hell Mode: Finale', 'The ultimate Three Chambers challenge.']
    ];
    document.querySelector('#games').innerHTML = levels.map(([file, name, description], i) => `
      <article>
        <h2>${i + 1}. ${name}</h2><p>${description}</p>
        <div class="actions"><a href="game.html?level=${encodeURIComponent(file)}">Play</a><a class="secondary" href="${file}" download="${file}.py">Python source</a></div>
      </article>`).join('');
  </script>
</body>
</html>
